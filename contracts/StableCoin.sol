...
contract StableCoin is ERC20 {
    ...
    function getSlippageFee(uint256 burn_amount) public view returns(uint256){
    ...
    function burnDXSmintBNB(uint256 amount) public {
        ...
        uint256 slippage_fee = getSlippageFee(amount);
        require(slippage_fee<500, "Burn too much DXS.");
        uint256 price = getBNBprice();
        uint256 _amount = (amount - amount.mul(slippage_fee).div(1000)).mul(10**8).div(price);
